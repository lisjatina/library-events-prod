package com.kafka.libraryeventsprod.producer;
...
public class LibraryEventProducer {
    ...
    @Autowired
    ObjectMapper objectMapper; // converts object to Json

    public void sendLibraryEvent(LibraryEvent libraryEvent) throws JsonProcessingException {

        Integer key = libraryEvent.getLibraryEventId();
        String value = objectMapper.writeValueAsString(libraryEvent);
        // ListenableFuture - something that will follow in future
        ListenableFuture<SendResult<Integer,String>> listenableFuture =  kafkaTemplate.sendDefault(key,value);
        listenableFuture.addCallback(new ListenableFutureCallback<SendResult<Integer, String>>() {
            @Override // if publish failed
            public void onFailure(Throwable ex) {
                handleFailure(key, value, ex);
            }

            @Override // if publish was successful
    ...
    private ProducerRecord<Integer, String> buildProducerRecord(Integer key, String value, String topic) {
        // headers needed in case if we want to send some additional information to the topics (meta-data about the message)
